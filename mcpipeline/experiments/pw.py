"""
Experiment to calculate pW couplings
"""
...
class PWConf(CouplingsConf):
  m: float

class PWRule(CouplingsRule[PWConf]):
  def __call__(
    self, 
    network: MMNetwork,
    ...
    def pfgw(src_t, src, dest_t, dest, **kwargs):
      ...
      return ot.pWasserstein(src, dest, m = m, M=M, **kwargs)
...
class PWTarget(CouplingsTarget[CouplingsConf]):
  def __init__(
    self, 
    name: str,
    cache_path: str,
