"""
Experiment to calculate fgw couplings
"""
...
class FGWRule(CouplingsRule[CouplingsConf]):
  ...
  def __call__(
    ...
    def gw(src_t, src, dest_t, dest, **kwargs):
      ...
      return ot.fGW(src, dest, M=M, **kwargs)
...
class FGWTarget(CouplingsTarget[CouplingsConf]):
  def __init__(
    self, 
    name: str,
    cache_path: str,
    ...
    **kwargs,
    ...
    super().__init__(
      ...
      rule = FGWRule(distance_only=distance_only),
      conf = {
        'num_random_iter': num_random_iter,
        'random_state': random_state,
      },
